...
const Dashboard = () => {
  ...
  useEffect(() => {
    const checkMetadata = async () => {
      console.log("[DASHBOARD] Checking metadata for user:", user?.email)
      try {
        const response = await fetch("/api/metadata/fetch", {
          credentials: "include",
        })

        console.log("[DASHBOARD] Metadata response status:", response.status)

        if (!response.ok) {
          console.log("[DASHBOARD] No metadata found (response not OK)")
          setIsLoading(false)
          return
        }
        ...
        if (data.data?.exists && data.data?.business_name) {
          console.log("[DASHBOARD] Metadata found!")
          setMetadata(data.data)
        } else {
          console.log("[DASHBOARD] No metadata in response")
        }
      } catch (error) {
    ...
    <div className="flex-1 p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <p className="text-sm text-zinc-400 mb-3 truncate">{metadata.website_url}</p>
          <a
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <p className="text-sm text-zinc-400 mb-3">Your AI assistant's knowledge</p>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <p className="text-sm text-zinc-400 mb-3">Configure your AI assistant</p>
          <span className="text-sm text-indigo-400 cursor-pointer hover:text-indigo-300">
            Edit settings →
          </span>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 rounded-2xl p-6">
          ...
          <p className="text-sm text-zinc-400 mb-3">Extend your chatbot's capabilities</p>
